...
const SceneBreakdown = ({ data }) => {
  ...
  return (
    <div style={{ marginBottom: '32px' }}>
      ...
      {/* Compact scene boxes: Scene 1 → Scene 2 → Scene 3 */}
      <div
        style={{
          display: 'flex',
          flexWrap: 'wrap',
      ...
      >
        {scenes.map((scene, idx) => (
          <span key={idx} style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            <button
              type="button"
              onClick={() => setSelectedSceneIndex(selectedSceneIndex === idx ? null : idx)}
              style={{
                padding: '10px 16px',
                borderRadius: 'var(--radius)',
                border: selectedSceneIndex === idx ? '2px solid var(--primary)' : '1px solid var(--border)',
                background: selectedSceneIndex === idx ? 'var(--primary-dim)' : '#171717',
                color: 'var(--text)',
                fontSize: '14px',
                fontWeight: 500,
                cursor: 'pointer',
      ...
      {/* Full details for selected scene only */}
      {selectedSceneIndex == null ? (
        ...
        >
          Click a scene above to view its details
        </div>
      ) : (
        (() => {
